...
import AdventureGame.Play
import AdventureGame.Load
import AdventureGame.UI

gameEngine :: Engine Game
gameEngine input game =
  case parseCommand input of
    Just command -> play command game
    Nothing -> Action (Print "I don't know how to do that") game

gameLoader :: Loader (Action Game)
gameLoader string =
  case load string of
    (Just room) -> Just $ showRoom (Game room)
    Nothing -> Nothing
